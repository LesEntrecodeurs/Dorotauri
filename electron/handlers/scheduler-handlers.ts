...
const SCHEDULER_METADATA_PATH = path.join(os.homedir(), '.dorotoring', 'scheduler-metadata.json');
...
function resolveLogPath(taskId: string): string {
  ...
  // Also check dorotoring plist format
  const dorotoringPlistPath = path.join(os.homedir(), 'Library', 'LaunchAgents', `com.dorotoring.scheduler.${taskId}.plist`);
  if (fs.existsSync(dorotoringPlistPath)) {
    try {
      const plistContent = fs.readFileSync(dorotoringPlistPath, 'utf-8');
      const stdOutMatch = plistContent.match(/<key>StandardOutPath<\/key>\s*<string>([^<]+)<\/string>/);
      if (stdOutMatch) return stdOutMatch[1];
    } catch { /* use default */ }
  }
...
async function getCLIPath(providerId: AgentProvider = 'claude'): Promise<string> {
  ...
  try {
    const settingsFile = path.join(os.homedir(), '.dorotoring', 'app-settings.json');
...
async function createLaunchdJob(
  ...
  const scriptPath = path.join(os.homedir(), '.dorotoring', 'scripts', `${taskId}.sh`);
  ...
  const label = `com.dorotoring.scheduler.${taskId}`;
...
async function createCronJob(
  ...
  const cronLine = `${schedule} ${scriptPath} # dorotoring-${taskId}`;
...
export function registerSchedulerHandlers(deps: SchedulerDeps): void {
        ...
        if (fs.existsSync(launchAgentsDir)) {
          try {
            ...
            for (const file of files) {
              if (!file.startsWith('com.claude.schedule.') && !file.startsWith('com.dorotoring.scheduler.')) continue;
              ...
              if (file.startsWith('com.claude.schedule.')) {
                ...
                taskId = file.replace('com.dorotoring.scheduler.', '').replace('.plist', '');
              }
  ...
  ipcMain.handle('scheduler:deleteTask', async (_event, taskId: string) => {
    try {
      ...
      if (os.platform() === 'darwin') {
        const labels = [
          `com.dorotoring.scheduler.${taskId}`,
        ...
        await new Promise<void>((resolve) => {
          ...
          getCron.on('close', () => {
            const newCron = existingCron
              .split('\n')
              .filter(line => !line.includes(`dorotoring-${taskId}`))
              .join('\n');

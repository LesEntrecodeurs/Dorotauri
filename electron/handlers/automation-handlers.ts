...
const AUTOMATIONS_DIR = path.join(os.homedir(), '.dorotauri');
...
async function createAutomationLaunchdJob(automation: Automation): Promise<void> {
  ...
  const logPath = path.join(os.homedir(), '.dorotauri', 'logs', `automation-${automation.id}.log`);
  const errorLogPath = path.join(os.homedir(), '.dorotauri', 'logs', `automation-${automation.id}.error.log`);
  ...
  const scriptPath = path.join(os.homedir(), '.dorotauri', 'scripts', `automation-${automation.id}.sh`);
  ...
  const label = `com.dorotauri.automation.${automation.id}`;
...
async function removeAutomationLaunchdJob(automationId: string): Promise<void> {
  const label = `com.dorotauri.automation.${automationId}`;
  const plistPath = path.join(os.homedir(), 'Library', 'LaunchAgents', `${label}.plist`);
  const scriptPath = path.join(os.homedir(), '.dorotauri', 'scripts', `automation-${automationId}.sh`);
...
export function registerAutomationHandlers(): void {
  ...
  ipcMain.handle('automation:run', async (_event, id: string) => {
    try {
      ...
      const scriptPath = path.join(os.homedir(), '.dorotauri', 'scripts', `automation-${id}.sh`);
      if (fs.existsSync(scriptPath)) {
  ...
  ipcMain.handle('automation:getLogs', async (_event, id: string) => {
    try {
      const logPath = path.join(os.homedir(), '.dorotauri', 'logs', `automation-${id}.log`);

...
export class ClaudeProvider implements CLIProvider {
  ...
  buildInteractiveCommand(params: InteractiveCommandParams): string {
    ...
    // Dorotoring's CLAUDE.md via ~/.dorotoring
    command += ` --add-dir '${os.homedir()}/.dorotoring'`;
...
unset CLAUDECODE
CLAUDE_CODE_ADDITIONAL_DIRECTORIES_CLAUDE_MD=1 "${params.binaryPath}" ${flags} --output-format stream-json --verbose --mcp-config "${params.mcpConfigPath}" --add-dir "${params.homeDir}/.dorotoring" -p '${params.prompt}' >> "${params.logPath}" 2>&1

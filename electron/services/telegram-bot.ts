...
function sendUnauthorizedMessage(chatId: string | number) {
  telegramBot?.sendMessage(chatId,
    ...
    `_Get the token from Dorotauri Settings → Telegram_`,
    { parse_mode: 'Markdown' }
  );
}
  ...
  try {
    ...
    telegramBot.onText(/\/auth\s+(.+)/, (msg, match) => {
      ...
      if (!appSettings.telegramAuthToken) {
        telegramBot?.sendMessage(chatId,
          ...
          '_Generate one in Dorotauri Settings → Telegram_',
          { parse_mode: 'Markdown' }
        );
        return;
      }
      ...
      if (providedToken === appSettings.telegramAuthToken) {
        ...
        telegramBot?.sendMessage(chatId,
          '❌ *Invalid token*\n\n' +
          '_Check your token in Dorotauri Settings → Telegram_',
          { parse_mode: 'Markdown' }
        );
      }
    });
    ...
    telegramBot.onText(/\/start_agent\s+(.+)/, async (msg, match) => {
      ...
      try {
        ...
        command += ` ${cliProvider.getAddDirFlag()} '${require('os').homedir()}/.dorotauri'`;
...
export async function sendToSuperAgent(chatId: string, message: string, attachedFiles?: string[]) {
  ...
  if (!superAgent) {
    telegramBot?.sendMessage(chatId,
      '👑 No Super Agent found.\n\nCreate one in Dorotauri first, or use /start\\_agent to start a specific agent.',
      { parse_mode: 'Markdown' }
    );
    return;
  }

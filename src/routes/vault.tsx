...
export default function VaultPage() {
  const [activeTab, setActiveTab] = useState<'dorotoring' | 'obsidian'>('dorotoring');
  ...
  return (
    <div className="h-[calc(100vh-7rem)] lg:h-[calc(100vh-3rem)] flex flex-col pt-4 lg:pt-6">
      ...
      <div className="flex items-center gap-2 mb-4 overflow-x-auto pb-2">
        <button
          onClick={() => setActiveTab('dorotoring')}
          className={`
            flex items-center gap-2 px-3 py-2 text-sm font-medium transition-all whitespace-nowrap
            ${activeTab === 'dorotoring'
              ? 'bg-foreground text-background'
              : 'bg-secondary text-muted-foreground hover:text-foreground border border-border'
            }
          `}
        >
          <img src="/dorotoring-logo.png" alt="Dorotoring" className="w-4 h-4 object-contain" />
          Dorotoring Vault
        </button>
        <button
      ...
      <div className="flex-1 min-h-0 overflow-hidden">
        {activeTab === 'dorotoring' ? <VaultView embedded /> : <ObsidianVaultView />}
      </div>
    </div>
  );
}

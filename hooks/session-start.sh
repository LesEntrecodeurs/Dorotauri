#!/bin/bash
# Session start hook for dorotoring
...
echo "[$(date)] SESSION_START hook. AGENT_ID=${CLAUDE_AGENT_ID:-unset} SESSION_ID=$SESSION_ID" >> /tmp/dorotoring-hooks.log
...
echo "[$(date)] SESSION_START curl result: $RESULT" >> /tmp/dorotoring-hooks.log

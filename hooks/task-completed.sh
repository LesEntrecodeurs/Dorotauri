#!/bin/bash
# TaskCompleted hook for dorotoring
...
echo "[$(date)] TASK_COMPLETED hook. AGENT_ID=${CLAUDE_AGENT_ID:-unset} SESSION_ID=$SESSION_ID" >> /tmp/dorotoring-hooks.log
...
# Notify Dorotoring that task is completed — sets status + sends native notification
